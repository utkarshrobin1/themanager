...
async def tmute_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"🔇 {mention(target)} has been muted for <b>{duration_str}</b>!\n"
...
async def cache_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """List first names of all users stored in DB (SQLite + MongoDB)."""
    # Gather from SQLite user_cache
    cursor.execute("SELECT first_name FROM user_cache ORDER BY first_name")
    sqlite_rows = cursor.fetchall()
    names = [row[0] for row in sqlite_rows if row[0] and row[0] != "Unknown"]

    # Also pull from MongoDB if available
    if mongo_users_col:
        try:
            mongo_docs = mongo_users_col.find({}, {"first_name": 1})
            for doc in mongo_docs:
                fname = _mongo_fname(doc)
                if fname and fname != "Unknown" and fname not in names:
                    names.append(fname)
        except Exception:
            pass

    if not names:
        return await update.message.reply_text("📭 No users cached yet.")
    ...
    for i in range(0, total, chunk_size):
        ...
        numbered = "\n".join(f"{i + j + 1}. {html.escape(n)}" for j, n in enumerate(chunk))
        header = f"👥 <b>Cached Users ({total} total)</b>\n\n" if i == 0 else ""
        await update.message.reply_text(
            f"{header}{numbered}",
            parse_mode=ParseMode.HTML,
        )
