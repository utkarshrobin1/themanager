...
"""
╔══════════════════════════════════════════════════════════╗
║     The Manager — Telegram Bot v2.0 [Optimized]        ║
...
Dependencies:
    pip install python-telegram-bot==20.7 pymongo dnspython aiosqlite
...
try:
    ...
    aiosqlite = None  # pip install aiosqlite
import logging
import re
import time
import json
...
logging.basicConfig(
    ...
    level=logging.WARNING,  # production — change to INFO for debugging
)
...
# ── WAL mode: better concurrency, no reader/writer blocking ──────────────────
...
# SQLite dirty flag — set True on every write; cleared by _sqlite_auto_commit
_sqlite_dirty: bool = False

# MongoDB write cooldown: {user_id -> last_write_ts}
# Prevents per-message hammering of MongoDB (5-minute cooldown per user)
_mongo_write_cooldown: dict = {}
_MONGO_COOLDOWN_SECS: float = 300.0   # 5 minutes

# Admin check cache: {chat_id -> (frozenset[admin_ids], expiry_ts)}
# Avoids a get_administrators() Telegram API call on every admin-gated command
_admin_cache: dict = {}
_ADMIN_CACHE_TTL: float = 300.0       # 5 minutes


def _mark_dirty() -> None:
    """Mark SQLite as needing a commit. Background task flushes every 10 s."""
    global _sqlite_dirty
    _sqlite_dirty = True


def _mongo_cooldown_ok(user_id: int) -> bool:
    """True (and record ts) if 5 min has passed since last Mongo write for user_id."""
    now = time.time()
    if now - _mongo_write_cooldown.get(user_id, 0.0) >= _MONGO_COOLDOWN_SECS:
        _mongo_write_cooldown[user_id] = now
        return True
    return False


# =============================================================================
#  BACKGROUND TASKS  (started via post_init hook in main())
# =============================================================================

async def _sqlite_auto_commit() -> None:
    """Flush SQLite writes every 10 seconds (batch instead of per-message)."""
    global _sqlite_dirty
    while True:
        ...
        if _sqlite_dirty:
            try:
                ...
                logger.warning("SQLite auto-commit error: %s", _e)


async def _cleanup_caches() -> None:
    """Evict expired entries from all in-memory caches every 5 minutes."""
    while True:
        await asyncio.sleep(300)
        now = time.time()

        # Expired admin-cache entries
        stale = [cid for cid, (_, exp) in list(_admin_cache.items()) if exp < now]
        for cid in stale:
            _admin_cache.pop(cid, None)

        # Old MongoDB cooldown entries (> 10 min old — safe to purge)
        cutoff = now - (_MONGO_COOLDOWN_SECS * 2)
        old = [uid for uid, ts in list(_mongo_write_cooldown.items()) if ts < cutoff]
        for uid in old:
            _mongo_write_cooldown.pop(uid, None)

        # Stale flood-cache entries (per-chat per-user, expire after 60 s idle)
        try:
            dead = [k for k, v in list(flood_cache.items())
                    if now - v.get("_ts", now) > 60]
            for k in dead:
                flood_cache.pop(k, None)
        except Exception:
            pass
...
async def is_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int = None) -> bool:
      """Check admin status with 5-minute in-memory cache (avoids repeated API calls)."""
      ...
      if cached:
          admin_ids, expiry = cached
          if now < expiry:
              return user_id in admin_ids

      # Cache miss or expired — refresh full admin list from Telegram
      try:
          admins = await chat.get_administrators()
          admin_ids = frozenset(m.user.id for m in admins)
          _admin_cache[chat.id] = (admin_ids, now + _ADMIN_CACHE_TTL)
          return user_id in admin_ids
      except (BadRequest, TelegramError):
          # Fallback: direct member check (not cached to avoid persisting errors)
          try:
...
def cache_user(user) -> None:
    """Cache user data — MongoDB PRIMARY (5-min cooldown), SQLite secondary.

    MongoDB is only written when the per-user cooldown has expired, preventing
    a Mongo round-trip on every single message.  SQLite is always updated and
    flushed to disk every 10 s by the _sqlite_auto_commit background task.
    """
    if not user or getattr(user, "is_bot", False):
        return
    # 1. Primary: MongoDB — throttled to once per 5 minutes per user
    if _mongo_cooldown_ok(user.id):
        mongo_upsert_user(user)
    # 2. Secondary: SQLite local cache — always write, batch-committed every 10 s
    uname = getattr(user, "username", None)
    try:
        cursor.execute(
...
def record_chat_member(chat_id: int, user) -> None:
    """Record a user as member of chat_id — MongoDB PRIMARY (cooldown), SQLite secondary.

    MongoDB is only written when the per-(user, chat) cooldown expires to avoid
    hammering the DB on every message.  SQLite is always updated and flushed every 10 s.
    """
    if not user or getattr(user, "is_bot", False):
        return
    # 1. Primary: MongoDB (persistent, throttled per user+chat composite key)
    _ck = user.id ^ (chat_id << 20)   # cheap composite key for cooldown slot
    _now = time.time()
    if _now - _mongo_write_cooldown.get(_ck, 0.0) >= _MONGO_COOLDOWN_SECS:
        _mongo_write_cooldown[_ck] = _now
        mongo_upsert_group_member(chat_id, user)
    # 2. Secondary: SQLite in-process cache — always write, batched commit every 10 s
    uname = getattr(user, "username", None)
    try:
        cursor.execute(
...
async def _post_init(app) -> None:
    """PTB post_init hook — starts background asyncio tasks after the event loop is up."""
    loop = asyncio.get_event_loop()
    loop.create_task(_sqlite_auto_commit(), name="sqlite-auto-commit")
    loop.create_task(_cleanup_caches(),    name="cleanup-caches")
    logger.warning("Background tasks started: sqlite-auto-commit (10s), cleanup-caches (5min)")
...
def main():
    ...
    print("╚══════════════════════════════════════════════════╝")
    logger.warning("Bot started — optimized: batch-commit 10s | admin-cache TTL 5min | mongo-cooldown 5min")
